...
const sourcePath = "C:/Games/Nihon/Kotoba Minna No Nihonggo 1/Kosakata_Minna_no_Nihongo_1-25.md";
const targetPath = "src/data/kotobaN5.ts";
...
for (const line of lines) {
  ...
  items.push({
    id: `n5-kotoba-${String(items.length + 1).padStart(3, "0")}`,
    type: "kotoba",
    level: "n5",
    chapter,
    prompt: kana,
...
export const kotobaN5Items: LearningItem[] = ${JSON.stringify(items, null, 2)};
`;

fs.writeFileSync(targetPath, body, "utf8");
console.log(`Imported ${items.length} Kotoba N5 items to ${targetPath}`);
